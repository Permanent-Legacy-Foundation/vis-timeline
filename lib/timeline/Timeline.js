 ...
 */
  ...
  getItemRange() {
    ...
    const pixelMargin = this.options.pixelMargin ? Number(this.options.pixelMargin) : 10;
    ...
    if (min != null && max != null) {
      ...
      util.forEach(this.itemSet.items, item => {
        ...
        if (this.options.rtl) {
          startSide  = start - (item.getWidthRight()  + pixelMargin) * factor;
          endSide = end   + (item.getWidthLeft() + pixelMargin) * factor;
        } else {
          startSide  = start - (item.getWidthLeft()  + pixelMargin) * factor;
          endSide = end   + (item.getWidthRight() + pixelMargin) * factor;
        }
      ...
      if (minItem && maxItem) {
        const lhs = minItem.getWidthLeft() + pixelMargin;
        const rhs = maxItem.getWidthRight() + pixelMargin;
