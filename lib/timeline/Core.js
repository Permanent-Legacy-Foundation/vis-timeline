...
class Core {
  ...
  setOptions(options) {
    if (options) {
      ...
      const fields = [
        ...
        'start', 'end', 'clickToUse', 'dataAttributes', 'hiddenDates', 'pixelMargin',
  ...
  _redraw() {
    ...
    if (options.orientation.item != 'top') {
      const newOffset = Math.max(props.centerContainer.height - props.center.height -
        props.border.top - props.border.bottom, 0);
      
      if (options.orientation.item == 'center') {
        offset += newOffset / 2
      } else {
        offset += newOffset
      }
