 ...
 */
  ...
  repositionY() {
    ...
    if (orientation == 'top') {
      ...
      const centerHeight = this.parent.itemSet.body.dom.center.offsetHeight;
      const containerHeight = this.parent.itemSet.body.dom.centerContainer.offsetHeight;
      const biggerHeight = Math.max(containerHeight, centerHeight);
      const itemSetHeight = this.parent.itemSet.props.height;
      const deadSpace = biggerHeight - itemSetHeight
      const lineHeight = biggerHeight - (deadSpace / 2) - this.boxY - (this.height || 0)
      const lineTop = this.boxY + (this.height || 0);

      lineStyle.height = `${lineHeight}px`;
      lineStyle.top = `${lineTop}px`;
      lineStyle.bottom = '';
